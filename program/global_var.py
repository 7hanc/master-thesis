...
def read_sample():
    ...
    df_co = pd.read_csv('./cp_ratio/cost.csv')   
    df_p = pd.read_csv('./cp_ratio/price.csv')      
...
def cap_req(c):
    ...
    cap = [int(i) for i in np.random.normal(c,110,size)] 
    req = [int(i) for i in np.random.normal(1000,110,size)] 
...
def co_p(c):
    ...
    co = [int(i) for i in np.random.normal(c,110,size)] 
    p = [int(i) for i in np.random.normal(1000,110,size)] 
